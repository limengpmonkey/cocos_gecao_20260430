...
export class Gun extends Component {
    ...
    @property
    speed: number = 0.5; //子弹飞行最大速度

    @property
    lifeTime: number = 0.5; //子弹飞行最长时间

    @property
    cycleTime: number = 5; //子弹发射间隔

    @property
    baseDamage: number = 20; //基础子弹伤害
    ...
    update(dt: number): void {
        ...
        if (this.isShoot) {
            this.isShoot = false;
            const intervalMultiplier = Math.max(0.1, BulletHell.inst?.normalAttackIntervalMultiplier ?? 1);
            const minInterval = Math.max(0, BulletHell.inst?.normalAttackMinInterval ?? 0);
            this.nextCycle = Math.max(this.cycleTime * intervalMultiplier, minInterval);
            ...
            bullet.damage = Math.max(1, Math.round(this.baseDamage * (Player.inst?.getDamageMultiplier() ?? 1)));
